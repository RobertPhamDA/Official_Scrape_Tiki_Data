...
def extract_comments_tiki():
    ...
    df_comment['insertedDate'] = pd.to_datetime('today').strftime('%Y-%m-%d %H:%M:%S')
    df_comment = df_comment.drop_duplicates(subset=['id'])
    return df_comment

def load_to_postgres_tiki_comment_detail(df_comment: pd.DataFrame):
